...
1. Set up the server:
   1. Run
      the `neelkamath/omni-chat:0.9.0` [Docker Hub image](https://hub.docker.com/repository/docker/neelkamath/omni-chat)
      .
...
1. Stop every instance of the server.
1. Wipe the DB because this version is backwards-incompatible.
1. Run
   the `neelkamath/omni-chat:0.9.0` [Docker Hub image](https://hub.docker.com/repository/docker/neelkamath/omni-chat). 
