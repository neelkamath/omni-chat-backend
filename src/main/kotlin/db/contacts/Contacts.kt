...
import com.neelkamath.omniChat.db.contacts.Contacts.contactId
...
object Contacts : IntIdTable() {
    ...
    /**
     * Saves the [ownerId]'s [contactIdList], ignoring existing contacts. The [ownerId] will be notified if they've
    ...
    fun create(ownerId: String, contactIdList: Set<String>) {
        ...
        newContacts.map { NewContact.buildFromUserId(it) }.forEach { notifyContactUpdate(ownerId, it) }
    }
     ...
     * notified of the [DeletedContact]s if they've [subscribeToContactUpdates].
     */
    fun delete(ownerId: String, contactIdList: List<String>) {
        ...
        contacts.forEach { notifyContactUpdate(ownerId, DeletedContact(it)) }
    }

    /**
     * Deletes every contact who owns, or is owned by, the given [userId]. Clients who have [subscribeToContactUpdates],
     * and have the [userId] in their contacts, will be notified of the [DeletedContact].
     */
    fun deleteUserEntries(userId: String) {
        notifyOfDeletedContact(userId)
        transact {
